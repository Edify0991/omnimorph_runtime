...
- `/humanoid/rl/mode_control`: `std_msgs/Int32`, reliable operator lifecycle and mode words.
- `/humanoid/rl/teleop`: `geometry_msgs/Twist`, best-effort velocity/yaw command.
- `/humanoid/rl/state`: `std_msgs/Float32MultiArray`, best-effort protocol-v2 robot telemetry.
...
`scripts/humanoid_ops_gui.py` provides the initial standalone Qt console:

- Publishes `/humanoid/rl/mode_control`.
...
```bash
ros2 run rl_master humanoid_ops_gui.py
```
...
python3 src/humanoid_rl_controller/rl_master/scripts/humanoid_ops_gui.py
```
