...
- `main()` in [rl_solver.cpp](${OMNIMORPH_RUNTIME_ROOT}/src/omnimorph_rl_controller/rl_master/rl_solver.cpp)
...
17. asynchronous telemetry thread publishes `/omnimorph/rl/state` at configured low frequency
...
- `main()` in `src/omnimorph_sim2sim/mujoco_sim2sim/src/main.cpp`
...
11. asynchronous telemetry thread publishes `/omnimorph/rl/state` at configured low frequency
12. `updateViewerFrameMirror()`
13. `updateViewerInspectorMirror(...)`
14. asynchronous viewer telemetry thread publishes:
15. `/omnimorph/sim2sim/mujoco_viewer_frame`
16. `/omnimorph/sim2sim/mujoco_viewer_inspector`
...
In the fused runtimes, `/omnimorph/rl/state` is now asynchronous telemetry only. It is not part of the internal control path anymore.
