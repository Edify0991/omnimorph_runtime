...
| --- | --- | --- | --- | --- | --- |
| `/omnimorph/rl/teleop` | input | joystick / teleop tools | fused `RL_solver`, fused `MujocoSimBridge` | sim2real, sim2sim | desired `vx / vy / yaw_rate` command |
| `/omnimorph/rl/mode_control` | input | operator tools / scripts | fused `RL_solver`, fused `MujocoSimBridge` | sim2real, sim2sim | deploy mode switch + lifecycle control words |
| `/imu/yesense` | input | IMU node | fused `RL_solver` | sim2real | base angular velocity / orientation input |
| `/omnimorph/rl/state` | output | fused `RL_solver`, fused `MujocoSimBridge` | external monitors / bags / debug tools | sim2real, sim2sim | low-frequency runtime telemetry / state observability |

Notes:

- In fused runtimes, `/omnimorph/rl/state` is asynchronous telemetry only.
- The fused controller path does not read `/omnimorph/rl/state` back through DDS.
...
| Topic | Direction | Producer | Consumer | Used In | Purpose |
| --- | --- | --- | --- | --- | --- |
| `/omnimorph/sim2sim/mujoco_viewer_frame` | output | fused `MujocoSimBridge` | `mujoco_sim_viewer_frontend.py` | sim2sim python_frontend | mirrored MuJoCo frame snapshot for Python viewer |
| `/omnimorph/sim2sim/mujoco_viewer_inspector` | output | fused `MujocoSimBridge` | `mujoco_sim_viewer_frontend.py` | sim2sim python_frontend | human-readable runtime inspector summary |
...
low-frequency telemetry:
  MujocoSimBridge -> /omnimorph/rl/state
  MujocoSimBridge -> /omnimorph/sim2sim/mujoco_viewer_frame
  MujocoSimBridge -> /omnimorph/sim2sim/mujoco_viewer_inspector
```
...
- sim2sim Python viewer interface:
  - `/omnimorph/sim2sim/mujoco_viewer_frame`
  - `/omnimorph/sim2sim/mujoco_viewer_inspector`
