...
- all 28 joints now use MuJoCo `<motor>` actuators in the sim-only XML
- when the controller is inactive in `HOLD`, the bridge latches the first settled `HOLD` pose and keeps that current pose with torque PD
- non-policy upper-body and waist joints are no longer forced toward a separate sim-only zero target while policy is running
...
- MuJoCo `<position>` actuators are now retuned dynamically at runtime: policy-controlled joints use the active mode profile `kps/kds/tau_limit`, and non-policy or inactive joints use `hold_kp/hold_kd/hold_torque_limit`
...
- non-policy upper-body and waist joints stay on the same torque backend as the legs
...
ros2 topic hz /omnimorph/rl/state
```
...
- Terminal 4 shows `/omnimorph/rl/state` publishing steadily
...
ros2 run mujoco_sim2sim mujoco_sim_viewer_frontend.py --ros-args \
  -p model_path:="${OMNIMORPH_RUNTIME_ROOT}/src/omnimorph_sim2sim/mujoco_sim2sim/models/jc01_fullbody_engineai_walk_sim2sim.xml" \
...
ros2 topic pub --once /omnimorph/rl/mode_control std_msgs/msg/Int32 "{data: 12}"
```
...
ros2 topic pub --once /omnimorph/rl/mode_control std_msgs/msg/Int32 "{data: 13}"
```
...
ros2 topic pub --once /omnimorph/rl/mode_control std_msgs/msg/Int32 "{data: 2000 + N}"
```
...
- `/omnimorph/rl/state` publishes steadily or the viewer shows motion updates
