...
ros2 run mujoco_sim2sim mujoco_sim_bridge --ros-args \
  ...
  -p rl_cfg_path:=/home/edify/Code/jc01_deploy/src/humanoid_rl_controller/rl_master/config/rl_cfg_amp_mjlab_jc01_full_body.yaml \
  -p startup_mode_id:=0 \
...
3. `startup_mode_id:=0` resolves to `amp_mjlab_jc01_full_body` from
   [rl_cfg_amp_mjlab_jc01_full_body.yaml](../../../humanoid_rl_controller/rl_master/config/rl_cfg_amp_mjlab_jc01_full_body.yaml).
...
1. ROS2 delivers `/humanoid/rl/mode_control` to the IO executor thread
