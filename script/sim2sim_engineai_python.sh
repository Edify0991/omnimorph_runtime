...
Options:
  ...
  --start-rl-controller <bool>    start_rl_controller launch arg (default: true)
...
print_banner "EngineAI Sim2Sim (Python Viewer Backend)"
...
if [[ -z "${PRECHECK_PYTHON}" ]]; then
  ...
  if [[ "${PRECHECK_PYTHON}" == "python3" ]] && [[ "${RESOLVED_PRECHECK_PYTHON}" == *"/anaconda"* || "${RESOLVED_PRECHECK_PYTHON}" == *"/miniconda"* || "${RESOLVED_PRECHECK_PYTHON}" == *"/mambaforge"* ]] && [[ -x "/usr/bin/python3" ]]; then
    ...
    log_warn "If you want conda precheck, pass absolute path via --precheck-python /path/to/conda/python."
  else
...
# Avoid ROS Python ABI mismatch when a conda python is first in PATH.
...
log_info "Launching sim2sim with python_interactive backend ..."
log_info "${LAUNCH_CMD[*]}"
exec "${LAUNCH_CMD[@]}"
